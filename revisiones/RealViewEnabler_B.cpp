...
std::string obtenerRenderer(const std::wstring& rutaRegistro) {
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, rutaRegistro.c_str(), 0, KEY_READ, &hKey) != ERROR_SUCCESS) {
        std::cerr << "Error: No se pudo abrir la clave del registro en la ruta especificada." << std::endl;
        return "";
    }
    ...
    if (RegQueryValueEx(hKey, L"renderer", NULL, NULL, (LPBYTE)value, &bufferSize) != ERROR_SUCCESS) {
        std::cerr << "Error: No se pudo obtener el valor 'renderer' del registro." << std::endl;
        RegCloseKey(hKey);
        return "";
    }
...
std::string crearContenidoReg(const std::string& version, const std::string& renderer) {
    ...
    if (renderer.find("NVIDIA") != std::string::npos) {
        ...
        std::cerr << "Error: GPU no compatible." << std::endl;
        return "";
    }
...
void guardarArchivoReg(const std::string& contenido) {
    ...
    if (regFile.is_open()) {
        ...
        std::cerr << "Error: No se pudo crear el archivo .reg." << std::endl;
    }
}
...
int main() {
    ...
    std::string version;
    std::cout << "Ingrese la versión de SolidWorks instalada (e.g., 2022): ";
    std::cin >> version;

    // Construir la ruta del registro
    std::wstring rutaRegistro = construirRutaRegistro(version);

    // Obtener el valor de "renderer" desde el registro
    ...
    if (!renderer.empty()) {
        ...
        if (contenidoReg.empty()) {
            std::cerr << "Error: No se pudo generar el contenido del archivo .reg." << std::endl;
            return 1;
        }
        ...
        std::cerr << "Error: No se pudo encontrar un GPU compatible en la versión de SolidWorks especificada." << std::endl;
    }
